...
const fieldCls =
  'h-11 w-full rounded-xl disabled:bg-white! disabled:opacity-100! disabled:text-[#0a0a0a]! border-[#E5E5E5] bg-white px-3.5 text-[15px] font-normal text-[#0A0A0A] shadow-[0_1px_2px_rgba(0,0,0,0.06)] dark:border-white/10 dark:bg-card dark:text-white'
...
function JsonBox({ value }) {
  return (
    <pre className="whitespace-pre-wrap break-all rounded-xl border border-dashed border-[#D4D4D4] bg-[#FAFAFA] p-4 font-mono text-[13px] leading-[20px] text-[#0A0A0A] dark:border-white/15 dark:bg-white/5 dark:text-white">
...
export default function AuditDetailModal({ row, onClose }) {
  ...
  return (
    <Dialog open={!!row} onOpenChange={(next) => !next && onClose()}>
      <DialogContent className="gap-0 rounded-2xl p-0 sm:max-w-[640px]">
        ...
        <div className="flex flex-col gap-5 px-6 pb-6 pt-2">
          <div>
            ...
            <Input disabled value={row.foydalanuvchi} className={fieldCls} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <Input disabled value={row.vaqtFull} className={fieldCls.replace('px-3.5', 'pl-9 pr-3.5')} />
              </div>
            </div>
            <div>
              <Label className={labelCls}>IP manzili</Label>
              <Input disabled value={row.ip} className={fieldCls} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-x-6 gap-y-5">
            <div>
              ...
              <Input disabled value={row.amal} className={fieldCls} />
            </div>
            <div>
              <Label className={labelCls}>Jadval nomi</Label>
              <Input disabled value={row.jadval} className={fieldCls} />
            </div>
            <div>
              <Label className={labelCls}>Yozuv raqami</Label>
              <Input disabled value={row.yozuv} className={fieldCls} />
            </div>
          </div>
