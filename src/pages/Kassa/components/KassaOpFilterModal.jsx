...
import { CASHIER_NAME, KASSAS } from '@/features/kassa/kassaMockData'
...
const CASHIERS = [CASHIER_NAME, 'Salmonov S.']

export default function KassaOpFilterModal({ open, onOpenChange, filters, onApply }) {
    ...
    >
      <DialogContent className="p-5 sm:max-w-[460px]">
        <DialogHeader className="flex flex-row items-center justify-between pb-1">
          ...
          <button
            type="button"
            onClick={() => setDraft(EMPTY_OP_FILTERS)}
        ...
        <div className="grid gap-4 py-1">
          ...
          <div>
            <Label className={labelCls}>Amal turi</Label>
            <Input placeholder="Masalan: Savdo to'lovi" value={draft.type} onChange={(e) => set('type', e.target.value)} className={inputCls} />
          </div>
        </div>
