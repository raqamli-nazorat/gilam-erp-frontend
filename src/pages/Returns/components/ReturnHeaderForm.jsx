import { RETURN_AGENTS, RETURN_COUNTERPARTIES, RETURN_TRANSPORTS, RETURN_WAREHOUSES } from '@/features/returns/returnsMockData'
...
export default function ReturnHeaderForm({ doc, exchangeRate, readOnly, onChange }) {
  return (
      ...
      <div className="grid flex-1 grid-cols-1 items-center gap-4 sm:grid-cols-5">
        ...
        <div>
          ...
          <Select value={doc.counterparty || '__none'} onValueChange={(v) => onChange({ counterparty: v === '__none' ? '' : v })} disabled={readOnly}>
            ...
            <SelectContent>
              ...
              {RETURN_COUNTERPARTIES.map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        ...
        <div>
          ...
          <Select value={doc.agent} onValueChange={(v) => onChange({ agent: v })} disabled={readOnly}>
            ...
            <SelectContent>
              {RETURN_AGENTS.map((a) => <SelectItem key={a} value={a}>{a}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        ...
        <div>
          ...
          <Select value={doc.warehouse} onValueChange={(v) => onChange({ warehouse: v })} disabled={readOnly}>
            <SelectTrigger className={fieldCls}><SelectValue /></SelectTrigger>
            <SelectContent>
              {RETURN_WAREHOUSES.map((w) => <SelectItem key={w} value={w}>{w}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
        ...
        <div>
          ...
          <Select value={doc.transport} onValueChange={(v) => onChange({ transport: v })} disabled={readOnly}>
            ...
            <SelectContent>
              {RETURN_TRANSPORTS.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}
            </SelectContent>
          </Select>
        </div>
      </div>
