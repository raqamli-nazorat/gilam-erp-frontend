    ...
    >
      {rows.map((o) => (
        <tr key={o.id} className="h-[60px] hover:bg-[#F9FAFB] dark:hover:bg-white/5">
          ...
          <td className={TD_NUM}>{formatNumber(o.qoldiq, 2)}</td>
          <td className={TD_NUM}>{formatNumber(o.qiymat, 0)}</td>
          <td className="px-4">
      ...
      <tr className="h-[52px] bg-[#F5F5F5] font-semibold dark:bg-white/5">
        ...
        <td className="px-4 text-right text-[13px] text-[#0A0A0A] dark:text-white">{formatNumber(s.qoldiq, 2)}</td>
        <td className="px-4 text-right text-[13px] text-[#0A0A0A] dark:text-white">{formatNumber(s.qiymat, 0)}</td>
        <td />
      </tr>
    </FilialSubShell>
  )
