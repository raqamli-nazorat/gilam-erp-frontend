...
export function emptyListFilters(fields) {
  const f = { __holat: '' }
  ;(fields ?? []).forEach((x) => { f[x.key] = '' })
  return f
}

export default function ListFilterModal({ open, onOpenChange, fields, filters, onApply }) {
  const [draft, setDraft] = useState(filters)
  const set = (k, v) => setDraft((d) => ({ ...d, [k]: v }))
  const all = [...(fields ?? []), { key: '__holat', label: 'Holat', options: ['Faol', 'Arxiv'] }]
    ...
    >
      <DialogContent className="p-5 sm:max-w-[520px]">
        <DialogHeader className="flex flex-row items-center justify-between pb-1">
          ...
          <button
            type="button"
            onClick={() => setDraft(emptyListFilters(fields))}
