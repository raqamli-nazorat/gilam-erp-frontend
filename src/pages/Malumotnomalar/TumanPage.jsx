  ...
  return (
    <div className="flex h-full flex-col gap-4">
      ...
      <DistrictModal
        ...
        countries={countries}
        regions={regions}
        regionsLoading={regionsStatus === 'loading'}
        onSave={saveDistrict}
        onDelete={() => {
          const rec = modalRec
          setModalRec(null)
...
// Eslatma: "Kodi" maydoni backend District modelida hali yo'q — forma saqlanadi va so'rovga
// qo'shib yuboriladi (backend qo'llab-quvvatlay boshlagach avtomatik ishlaydi). "Davlat" select
// faqat "Viloyat"ni kaskad tarzda filtrlash uchun — API'ga faqat "viloyat" (region) yuboriladi.
function DistrictModal({ open, onOpenChange, record, countries, regions, regionsLoading, onSave, onDelete }) {
  const isEdit = !!record
  ...
  useEffect(() => {
    if (!open) return
    const initialRegion = record ? regions.find((r) => r.id === record.regionId) : null
    setName(record?.name ?? '')
    setCountry(initialRegion?.countryId ?? countries[0]?.id ?? '')
  ...
  const regionOptions = useMemo(
    () => (country ? regions.filter((r) => r.countryId === country) : regions),
    [regions, country]
  )

  const canSave = name.trim().length > 1 && !!region
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="gap-0 overflow-hidden p-0 sm:max-w-[480px]">
        <DialogHeader className="flex flex-row items-center justify-between px-5 pt-5">
          <DialogTitle className="text-[17px] font-semibold leading-[24px] tracking-[-0.2px] text-[#0A0A0A] dark:text-white">
            {isEdit ? 'Tahrirlash' : 'Yangi tuman'}
          </DialogTitle>
        </DialogHeader>
        <div className="grid grid-cols-2 gap-4 px-5 py-4">
          <div className="col-span-2">
            <Label className={labelCls}>Tuman nomi</Label>
            <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Masalan: Payariq" className={fieldCls} />
          </div>
          <div>
            <Label className={labelCls}>Davlat</Label>
            <Select
              value={country || '__none'}
              onValueChange={(v) => {
                setCountry(v === '__none' ? '' : v)
                setRegion('')
              }}
            >
          ...
          <div>
            ...
            <Select value={region || '__none'} onValueChange={(v) => setRegion(v === '__none' ? '' : v)}>
              <SelectTrigger className={fieldCls}>
                <SelectValue>
                  {(v) => {
                    if (v === '__none') return <span className="text-[#737373]">Tanlang</span>
                    return regionOptions.find((r) => r.id === v)?.name ?? ''
                  }}
                </SelectValue>
              </SelectTrigger>
              <SelectContent>
        ...
        <DialogFooter className="mx-0 mb-0 mt-0 flex items-center gap-2 border-0 bg-[#F5F5F5] px-5 py-4 dark:bg-white/5 sm:flex-row sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              disabled={!canSave}
              onClick={() => {
                onSave({ name: name.trim(), region, kodi })
                onOpenChange(false)
              }}
